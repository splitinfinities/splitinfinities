	...
	<div class="entry-social">
		...
		<?php while ( have_rows('sdo_social_share', 'options') ) : the_row(); ?>
			<?php if (get_sub_field( 'platform' ) === "twitter"): ?>
				<a target="_blank" href="https://twitter.com/intent/tweet?text=<?php echo get_the_title(); ?>&url=<?php echo get_the_permalink(); ?>" class="<?php echo get_sub_field( 'platform' ); ?>" title="Share this on <?php get_sub_field( 'platform' ) ?>"><?php partial('images/social', get_sub_field( 'platform' ) ); ?></a>
