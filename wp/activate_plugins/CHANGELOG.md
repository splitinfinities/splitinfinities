...
## 2.5.2 (2015-07-15)
* Hot Fix: fixes potential `Fatal error: Call to protected TNEIGHBORHOOD_Plugin_Activation::__construct()` error and other compatibility issues when both TGMPA 2.5+ as well as TGMPA 2.3.6- would be loaded by different themes and plugins.
...
* Hot Fix: fixes potential `Fatal error: Call to undefined method TNEIGHBORHOOD_Utils::validate_bool()` errors caused by a conflict with the Soliloquy plugin.
...
* Fixed: _"PHP Fatal error: Class 'TNEIGHBORHOOD_Bulk_Installer' not found"_ [#185] Thanks [Chris Talkington] for reporting.
...
* Added new class: `TNEIGHBORHOOD_Plugin_Installer` to prepare for must-use plugin support
...
* Added 3 new classes (all extensions of existing WordPress classes): `TGMPA_List_Table` for outputting required/recommended plugins in a familiar table format, `TNEIGHBORHOOD_Bulk_Installer` for bulk installing plugins and `TNEIGHBORHOOD_Bulk_Installer_Skin` for skinning the bulk install process
