...
class SENDO {
	...
	public function prepare_output() {
		...
		if ($twitter_card !== 'none') {
			$this->set_twitter_card('card', $twitter_card)
				 ...
				 ->set_twitter_card('site', get_field('sdo_twitter_card_site', 'options'))
				 ->set_twitter_card('description', substr(get_the_excerpt(), 0, 140) )
			...
			if ( $twitter_card === 'gallery' ) {
				$gallery = (get_field('sdo_twitter_card_gallery')) ? get_field('sdo_twitter_card_gallery') : get_field('sdo_twitter_card_gallery', 'options');
			...
			if ( $twitter_card === 'app' ) {
				$app_country = (get_field('sdo_twitter_card_app_country')) ? get_field('sdo_twitter_card_app_country') : get_field('sdo_twitter_card_app_country', 'options');
			...
			if ( $twitter_card === 'player' ) {
				...
				$all_video_details = (get_field('sdo_twitter_card_app_video')) ? get_field('sdo_twitter_card_app_video') : get_field('sdo_twitter_card_app_video', 'options');
			...
			if ( $twitter_card === 'product' ) {
				$all_product_details = (get_field('sdo_twitter_card_labels_and_data')) ? get_field('sdo_twitter_card_labels_and_data') : get_field('sdo_twitter_card_labels_and_data', 'options');
	...
	public function output($type) {
		...
		if ($type === 'meta') {
			...
			$tags = $this->tags;
			$rss = $this->rssfeeds;
			include "sendo/meta.php";
		}

		elseif ($type === 'scripts') {
			$js = $this->js;
			include "sendo/scripts.php";
		}

		elseif ($type === 'style') {
			$css = $this->css;
			include "sendo/style.php";
		}
...
/**
 * Helper function to call this instantiation.
